...
public class Player : Entity
{
    ...
    private void TryInteract()//进行交互
    {
        Transform closest = null;//使用Transform方便检测距离
        float closestDistance = Mathf.Infinity;//最近距离初始化为无穷大
        Collider2D[] objectsAround = Physics2D.OverlapCircleAll(transform.position, 1f);//检测目标周围的对象

        foreach (var target in objectsAround)//遍历检测到的对象
        {
            IInteractable interactable = target.GetComponent<IInteractable>();//检查该对象身上是否有接口组件
            if (interactable == null) continue;//没有的话就略过

            float distance = Vector2.Distance(transform.position, target.transform.position);//如果有接口组件（可以交互),就记录该对象与玩家之间的距离

            if (distance < closestDistance)//遍历迭代得出最近的距离和对象
            {
        ...
        if (closest == null)//如果遍历完了还是null（都没法交互）就返回
            return;

        closest.GetComponent<IInteractable>().Interact();//调用最近对象的接口
    }
